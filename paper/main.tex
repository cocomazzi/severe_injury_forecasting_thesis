...
To evalutate for forecastability, we computed the percentage of days with no reports over the total temporal range for each state (mean value across all states=0,62). Given the data sparsity at the daily level, we aggregated the data to a lower resolution.
...
A common approach to normalize a skewed distribution is to apply a log transformation to the target feature. However, because our target is already normalized by workforce size and because the target naturally included zero values (days with no report at all), we do not apply further transformations to our target (cit. ealing with Logs and Zeros in Regression Models).

To assess time serie components, we first visually inspect the mean monthly time series for hospitalization risk across all states in the panel. Figure~\ref{fig:monthly_mean_hosprisk} shows seasonal fluctuations, with regular upwards trends in the summer and a general decline after the COVID period.
...
A more formal test for temporal components is provided by a seasonal-trend decomposition (STL).
...
Target variable 
& \texttt{Hospitalization}: sum of hospitalizations for each state at a monthly frequency.\\[4pt]
