...
NOTE: \textit{I basically removed all features in the dataset, with the exception of: date and state, and for sq2.1, i feature engineered the naics mix. The target is Hospitalization (repeating the same experiments for Amputation is tedious). All other input features are autoregressive features (we need these to frame the problem as a supervised regression for ML regressors) and calendar features. Maybe we could better use our dataset???}
...
--- I DROPPED THIS PART: \textit{adjusting for population makes it actually harder to interpret the forecasts. For example, what does a 0.22 RMSE mean on the target Hospitalization per 100000 employees per month/week? I switched to counts of hospitalizations (e.g., a 3.5 RMSE means the model is off by 3.5 hospitalizations per month/week)}
